...
const calculateResult = ({ number, board }: { number: number; board: any }) =>
  number * board.undrawn.reduce((total, number) => total + number, 0);

export const partOne = pipe(parseInput, ({ drawnPool, dimensions, boards }) => {
...
export const partTwo = pipe(parseInput, ({ drawnPool, dimensions, boards }) => {
  let winningBoards = [];
